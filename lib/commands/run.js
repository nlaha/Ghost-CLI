...
class RunCommand extends Command {
    run() {
        ...
        if (process.stdin.isTTY) {
            this.ui.log(
                "The `ghost run` command is used by the configured Ghost process manager and for debugging. " +
                    "If you're not running this to debug something, you should run `ghost start` instead.",
                "yellow"
            );
        }
    ...
    useGhostUser(instance) {
        ...
        this.child = spawn(
            "sudo",
            `-E -u ghost ${process.execPath} current/index.js`.split(" "),
            {
                cwd: instance.dir,
                stdio: "inherit",
            }
    ...
    useDirect(instance) {
        ...
        this.child.on("message", (message) => {
            ...
            // NOTE: Backwards compatibility of https://github.com/nlaha/Ghost/pull/9440
            setTimeout(() => {
