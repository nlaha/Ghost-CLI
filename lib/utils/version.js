...
const utils = {
    async loadVersions(includePrerelease = false) {
        ...
        const versions = Object.keys(result.versions)
            .filter((v) =>
                semver.satisfies(v, MIN_RELEASE, { includePrerelease })
            )
            .sort(semver.rcompare);

        const deprecations = Object.keys(result.versions).reduce((obj, v) => {
    ...
    checkCustomVersion(version, versions, activeVersion, opts = {}) {
        ...
        if (!semver.satisfies(parsed, MIN_RELEASE)) {
            throw new CliError({
                message:
                    "Ghost-CLI cannot install versions of Ghost less than 1.0.0",
                log: false,
            });
        }
        ...
        if (!opts.zip && opts.v1 && parsed.major > 1) {
            throw new CliError({
                message:
                    "The --v1 flag was provided, but the custom version specified was v2 or greater",
        ...
        if (activeVersion && semver.lt(parsed, activeVersion)) {
            const message = opts.zip
                ? "Version in zip file"
                : "The custom version specified";
    ...
    checkActiveVersion(
        ...
        if (opts.v1 && activeMajor > 1) {
            throw new CliError({
                message:
                    "The --v1 flag was provided, but the current version of Ghost is v2 or greater",
        ...
        if (
            activeVersion !== latestMajor &&
            ["major", "premajor"].includes(
                semver.diff(activeVersion, versionToUse)
            )
        ) {
    ...
    async resolveVersion(
        ...
        const latestVersion = opts.v1
            ? versions.latestMajor.v1
            : versions.latest;
        ...
        if (customVersion) {
            const normalizedVersion =
                versions.latestMajor[customVersion.replace(/^v?/, "v")] ||
                customVersion;
            version = utils.checkCustomVersion(
                normalizedVersion,
                versions.all,
                activeVersion,
                opts
...
*/
        ...
        return utils.checkActiveVersion(
            activeVersion,
            latest,
            versions.latestMajor,
            opts
        );
    },
    ...
    async versionFromZip(zipPath, activeVersion = null, opts = {}) {
        ...
        if (!fs.existsSync(zipPath) || path.extname(zipPath) !== ".zip") {
            return Promise.reject(
                new SystemError("Zip file could not be found.")
            );
        }
        ...
        try {
            ...
            return Promise.reject(
                new SystemError(
                    "Zip file does not contain a valid package.json."
                )
            );
        }
        ...
        if (pkg.name !== "ghost") {
            return Promise.reject(
                new SystemError("Zip file does not contain a Ghost release.")
            );
        }

        if (semver.lt(pkg.version, "1.0.0")) {
            return Promise.reject(
                new SystemError("Zip file contains pre-1.0 version of Ghost.")
            );
        }
        ...
        if (
            ...
            return Promise.reject(
                new SystemError(
                    "Zip file contains a Ghost version incompatible with the current Node version."
                )
            );
        }
        ...
        if (
            pkg.engines &&
            pkg.engines.cli &&
            !semver.satisfies(cliPackage.version, pkg.engines.cli, {
        ...
        return utils.resolveVersion(pkg.version, activeVersion, {
            ...opts,
            zip: true,
        });
    },
