...
`.trim()
        ...
        { borderColor: "yellow", align: "center" }
    );

const ghostDeprecated = () =>
    boxen(
        chalk.yellow(
            `
...
async function deprecationChecks(ui, system) {
    ...
    const showGhostDeprecation = (await system.getAllInstances(false)).some(
        (instance) => semver.lt(instance.version, "3.0.0")
    );
